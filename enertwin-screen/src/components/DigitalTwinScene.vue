...
let gridMaterialRef: THREE.Material | null = null
const alarmDetailVisible = ref(false)
const raycaster = new THREE.Raycaster()
const pointer = new THREE.Vector2()
const alarmPickables: THREE.Object3D[] = []
let alarmWorldPosition: THREE.Vector3 | null = null
...
const campusBuildingNames: Record<string, string> = {
  a1: 'A座 综合楼',
  b2: 'B座 实验楼',
  c3: 'C座 研发中心',
  d4: 'D座 展厅',
...
const emphasisObjects: Record<string, THREE.Object3D[]> = {
  pv: [],
  storage: [],
  charging: [],
  building: [],
...
watch(
  () => [props.storageSoc, props.pvPowerKw, props.loadPowerKw, props.chargerPowerKw],
  () => updateLabels(),
)
...
function initScene(host: HTMLDivElement) {
  ...
  resizeObserver = new ResizeObserver(() => {
    if (!renderer || !labelRenderer || !sceneRef.value) return
...
function buildSky() {
  ...
  const horizon = new THREE.Mesh(
    new THREE.RingGeometry(58, 96, 160),
    new THREE.MeshBasicMaterial({
...
function buildShowroomStage() {
  ...
  const campusGlow = new THREE.Mesh(
...
function buildAerialGround() {
  ...
  const base = new THREE.Mesh(
    new THREE.BoxGeometry(68, 0.5, 40),
  ...
  gridMaterialRef = grid.material as THREE.Material
...
function addOuterDistrict(x: number, z: number, width: number, depth: number, color: number) {
...
function addOuterRoad(x: number, z: number, length: number, width: number, rotation: number) {
  const road = new THREE.Mesh(
    new THREE.BoxGeometry(width, 0.045, length),
    new THREE.MeshStandardMaterial({ color: 0x111b26, roughness: 0.78, metalness: 0.08 }),
  )
  ...
  const line = new THREE.Mesh(
    new THREE.BoxGeometry(0.045, 0.018, length * 0.92),
    new THREE.MeshBasicMaterial({ color: 0xffdd72, transparent: true, opacity: props.nightMode ? 0.56 : 0.42 }),
  )
...
function addCityDistrict(x: number, z: number, width: number, depth: number, count: number) {
...
function addWorldNeonCap(x: number, z: number, width: number, depth: number, y: number) {
  const points = [
...
function addDarkCityCarpet() {
  ...
  for (let ix = -5; ix <= 5; ix += 1) {
    for (let iz = -4; iz <= 4; iz += 1) {
      ...
      const z = iz * 12
      if (x > -38 && x < 31 && z > -24 && z < 20) continue
      if (Math.abs(x) > 82 || Math.abs(z) > 58) continue
...
function addFrontBoulevard() {
  const road = new THREE.Mesh(
    new THREE.BoxGeometry(116, 0.05, 8.4),
    new THREE.MeshStandardMaterial({ color: 0x121923, roughness: 0.72, metalness: 0.08 }),
  )
...
function addUrbanTraffic() {
  ...
  cars.forEach(([x, y, z, rotation], index) => {
    const car = new THREE.Mesh(
      new THREE.BoxGeometry(0.72, 0.16, 0.34),
      index % 2 === 0 ? carMaterialA : carMaterialB,
    )
    ...
    updaters.push((elapsed) => {
      ...
      if (rotation === 0 || rotation === Math.PI) car.position.z = z + offset
      else car.position.x = x + offset
    })
  })
}
...
function addCrosswalk(x: number, z: number, rotation: number) {
...
function addGroundPlate(x: number, z: number, width: number, depth: number, color: number, height = 0.06) {
  const material = new THREE.MeshStandardMaterial({
    color,
    roughness: 0.74,
    metalness: 0.04,
  ...
  const edge = new THREE.LineSegments(
    ...
    new THREE.LineBasicMaterial({ color: 0x173a48, transparent: true, opacity: 0.32 }),
  )
  edge.position.copy(plate.position)
  scene.add(edge)
}

function addPavingPlate(x: number, z: number, width: number, depth: number) {
  const material = new THREE.MeshStandardMaterial({
  ...
  const edge = new THREE.LineSegments(
    new THREE.EdgesGeometry(plate.geometry),
    new THREE.LineBasicMaterial({ color: 0x4ae7ff, transparent: true, opacity: props.nightMode ? 0.28 : 0.16 }),
  )
  edge.position.copy(plate.position)
  scene.add(edge)
...
function buildSurroundings() {
  for (let i = 0; i < 72; i += 1) {
    ...
    const tower = new THREE.Mesh(
      new THREE.BoxGeometry(1.6 + Math.random() * 2.1, h, 1.4 + Math.random() * 2.2),
      new THREE.MeshStandardMaterial({
    ...
    if (i % 3 === 0) {
      const cap = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 0x7ce5ff, transparent: true, opacity: props.nightMode ? 0.22 : 0.1 }),
      )
...
function createBuilding(item: { x: number; z: number; w: number; d: number; h: number; facade: number }) {
  const group = new THREE.Group()
  group.position.set(item.x, 0, item.z)

  const podium = new THREE.Mesh(
    new THREE.BoxGeometry(item.w * 1.16, 0.22, item.d * 1.14),
    new THREE.MeshStandardMaterial({
  ...
  const podiumInset = new THREE.Mesh(
    new THREE.BoxGeometry(item.w * 1.02, 0.035, item.d),
    new THREE.MeshBasicMaterial({
      color: 0x7ce5ff,
      transparent: true,
  ...
  const podiumEdge = new THREE.LineSegments(
    new THREE.EdgesGeometry(podium.geometry),
    new THREE.LineBasicMaterial({ color: 0x4ae7ff, transparent: true, opacity: props.nightMode ? 0.24 : 0.14 }),
  )
  ...
  const roof = new THREE.Mesh(
    new THREE.BoxGeometry(item.w * 1.03, 0.14, item.d * 1.03),
    new THREE.MeshStandardMaterial({
      color: 0x10263c,
      roughness: 0.36,
      metalness: 0.24,
...
function addBuildingPulseShell(group: THREE.Group, width: number, depth: number, height: number) {
...
function addWindowGrid(group: THREE.Group, width: number, depth: number, height: number) {
  const material = new THREE.MeshStandardMaterial({
    color: 0x89c7e8,
    emissive: 0x1d77a8,
...
function addWindow(group: THREE.Group, material: THREE.Material, x: number, y: number, z: number, rotationY: number) {
...
function addFacadeLightStrips(group: THREE.Group, width: number, depth: number, height: number) {
...
function addRoofNeonFrame(group: THREE.Group, width: number, depth: number, y: number) {
  const points = [
...
function addRoofEquipment(group: THREE.Group, width: number, depth: number, height: number) {
...
function createSolarArray(x: number, y: number, z: number, width: number, depth: number) {
  const group = new THREE.Group()
...
function buildStorageStation() {
  ...
  const pad = new THREE.Mesh(
    new THREE.BoxGeometry(7.8, 0.12, 3.8),
    new THREE.MeshStandardMaterial({ color: 0x334958, roughness: 0.7 }),
  )
  pad.position.y = 0.06
  group.add(pad)
  ...
  for (let i = 0; i < 5; i += 1) {
    const container = new THREE.Mesh(
      new THREE.BoxGeometry(1.15, 1.25, 2.25),
      new THREE.MeshStandardMaterial({
        color: 0x1c5f8a,
        metalness: 0.35,
        roughness: 0.32,
...
function buildChargingStation() {
  ...
  const canopy = new THREE.Mesh(
    new THREE.BoxGeometry(6.8, 0.18, 4.1),
    new THREE.MeshStandardMaterial({
      color: 0x18365a,
      emissive: 0x8a6cff,
  ...
  for (let i = 0; i < 8; i += 1) {
    const pile = new THREE.Mesh(
      new THREE.BoxGeometry(0.2, 0.82, 0.18),
      new THREE.MeshStandardMaterial({
        color: 0x351b78,
    ...
    if (i % 2 === 0) addCar(group, pile.position.x + 0.55, 0.12, pile.position.z + 0.3, i)
  }
...
function addFacilityHalo(x: number, z: number, radiusX: number, radiusZ: number, color: number, kind: string) {
  const group = new THREE.Group()
...
function buildParkingLot(x: number, z: number, rotation = 0) {
  const group = new THREE.Group()
  group.position.set(x, 0, z)
  group.rotation.y = rotation
  ...
  const entrance = new THREE.Mesh(
    new THREE.BoxGeometry(1.4, 0.012, 4.8),
    new THREE.MeshBasicMaterial({ color: 0x65b8ff, transparent: true, opacity: 0.18, blending: THREE.AdditiveBlending }),
  )
...
function addCar(group: THREE.Group, x: number, y: number, z: number, index: number) {
  const car = new THREE.Group()
  const body = new THREE.Mesh(
    new THREE.BoxGeometry(0.86, 0.24, 0.45),
    new THREE.MeshStandardMaterial({ color: index % 2 === 0 ? 0x7ce5ff : 0x8a6cff, roughness: 0.28, metalness: 0.24 }),
  )
...
function addTreeRows() {
  ...
  positions.forEach(([x, z], index) => {
    const treeX = x
    const treeZ = z
    if (isInsideLake(treeX, treeZ) || isInsideRoad(treeX, treeZ)) return
...
function isInsideRoad(x: number, z: number) {
...
function addSkywalk(x1: number, z1: number, x2: number, z2: number, y: number) {
  const dx = x2 - x1
  const dz = z2 - z1
  ...
  const body = new THREE.Mesh(
    new THREE.BoxGeometry(0.72, 0.28, length),
    new THREE.MeshStandardMaterial({
      color: 0x16314d,
      roughness: 0.34,
      metalness: 0.24,
  ...
  const floorGlow = new THREE.Mesh(
    new THREE.BoxGeometry(0.82, 0.026, length * 0.96),
    new THREE.MeshBasicMaterial({
      color: 0x65b8ff,
      transparent: true,
  ...
  for (const z of [-length / 2 + 0.12, length / 2 - 0.12]) {
    const deck = new THREE.Mesh(new THREE.BoxGeometry(1.26, 0.18, 0.72), body.material as THREE.Material)
...
function addCampusTreeGroves() {
  ...
  pocketTrees.forEach(([x, z], index) => {
    if (isInsideLake(x, z) || isInsideRoad(x, z)) return
...
function addRoad(x: number, y: number, z: number, width: number, length: number, rotation: number) {
  const road = new THREE.Mesh(
    new THREE.BoxGeometry(width, 0.045, length),
    new THREE.MeshStandardMaterial({
      color: 0x162531,
  ...
  const center = new THREE.Mesh(
    new THREE.BoxGeometry(0.065, 0.022, length * 0.88),
    new THREE.MeshBasicMaterial({ color: 0xffd86a, transparent: true, opacity: 0.8, blending: THREE.AdditiveBlending }),
  )
...
function addCampusBoundaryGlow() {
  ...
  const line = new THREE.Line(
    new THREE.BufferGeometry().setFromPoints(points),
    new THREE.LineBasicMaterial({ color: 0x22f59a, transparent: true, opacity: props.nightMode ? 0.48 : 0.28 }),
  )
  scene.add(line)
}
...
function addFenceSegment(x1: number, z1: number, x2: number, z2: number) {
  const dx = x2 - x1
  const dz = z2 - z1
  const length = Math.sqrt(dx * dx + dz * dz)
  const angle = Math.atan2(dx, dz)
  ...
  for (let i = 0; i <= postCount; i += 1) {
    ...
    if (i % 4 === 0) {
      const light = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 0x69f7ff, transparent: true, opacity: 0.8 }),
      )
...
function addCampusGate(x: number, z: number) {
  const group = new THREE.Group()
  group.position.set(x, 0, z)
  ...
  const entryPath = new THREE.Mesh(
    new THREE.BoxGeometry(5.8, 0.028, 4.2),
    new THREE.MeshBasicMaterial({
      color: 0x00e5ff,
      transparent: true,
...
function addWater(x: number, z: number, width: number, depth: number) {
...
function addBasketballCourt(x: number, z: number) {
  const group = new THREE.Group()
  group.position.set(x, 0, z)
  group.rotation.y = -0.04

  const court = new THREE.Mesh(
    new THREE.BoxGeometry(5.8, 0.035, 3.6),
    new THREE.MeshStandardMaterial({
      color: 0x315f76,
      roughness: 0.72,
      metalness: 0.08,
...
function addSportsField(x: number, z: number) {
  const field = new THREE.Mesh(
    new THREE.BoxGeometry(9.6, 0.035, 5.6),
    new THREE.MeshStandardMaterial({
      color: 0x238d58,
      roughness: 0.92,
...
function addFlow(points: THREE.Vector3[], color: number, kind: string) {
  ...
  for (let i = 0; i < 5; i += 1) {
    const particle = new THREE.Mesh(
      new THREE.SphereGeometry(i % 2 === 0 ? 0.13 : 0.085, 16, 16),
...
function makeAlarmBeacon() {
  ...
  const beam = new THREE.Mesh(
    new THREE.CylinderGeometry(0.07, 0.22, 3.1, 32, 1, true),
    beamMaterial,
  )
...
function makeLabel(id: string, html: string) {
  const element = document.createElement('div')
...
function updateLabels() {
  ...
  if (storage) storage.innerHTML = `储能站<b>${Math.round(props.storageSoc)}% SOC</b>`
  const charging = labels.get('charging')
  if (charging) charging.innerHTML = `充电站<b>${Math.round(props.chargerPowerKw)} kW</b>`
}
...
function applyTheme() {
  ...
  scene.fog = new THREE.FogExp2(props.nightMode ? 0x061426 : 0xb6d8f0, props.nightMode ? 0.009 : 0.008)
  if (renderer) renderer.toneMappingExposure = props.nightMode ? 1.22 : 1.05
  if (groundMaterial) {
...
function applySceneMode() {
  if (sceneRef.value) sceneRef.value.dataset.mode = props.mode
  ...
  const cameraTargets: Partial<Record<SceneMode, { position: THREE.Vector3; target: THREE.Vector3 }>> = {
...
function handleSceneClick(event: MouseEvent) {
  ...
  if (alarmWorldPosition) {
    ...
    if (distance < 110) alarmDetailVisible.value = true
  }
}
...
<template>
  <div ref="sceneRef" class="digital-twin-scene">
    <article v-if="alarmDetailVisible" class="alarm-detail-card">
      <header>
        <span>一级告警</span>
        <button type="button" title="关闭告警详情" @click="alarmDetailVisible = false">×</button>
      </header>
      <strong>PCS-02 储能 PCS 温度越限</strong>
      <dl>
        <div>
          <dt>位置</dt>
          <dd>C3 研发中心屋顶储能侧</dd>
