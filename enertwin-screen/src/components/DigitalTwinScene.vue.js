...
let decorativeAssetsReady = Promise.resolve();
...
async function addGroundCarpet() {
    ...
    const carpet = new THREE.Mesh(new THREE.PlaneGeometry(112, 112), new THREE.MeshStandardMaterial({
        map: texture,
        color: 0x738b9b,
        emissive: 0x061a2d,
...
function addDataSky() {
    ...
    for (let index = 0; index < 56; index += 1) {
        ...
        const beam = new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(x, 0, z), new THREE.Vector3(x, height, z)]), beamMaterial);
        scene.add(beam);
    }
}
function addCampusTelemetry() {
    ...
    const beam = new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0, 0.45, 0), new THREE.Vector3(0, 8.8, 0)]), beamMaterial);
    ...
    try {
        ...
        modelRoot.traverse((child) => {
            ...
            materials.forEach((material) => {
                const pbr = material;
                if (material.name.includes('UV_BAKED_FACADE'))
                    pbr.emissiveIntensity = 0.48;
                if (material.name.includes('WINDOW'))
                    pbr.emissiveIntensity = 0.58;
                if (material.name.includes('NEON'))
                    pbr.emissiveIntensity = 2.08;
                if (material.name.includes('SOLAR_PV'))
                    pbr.emissiveIntensity = 0.48;
                if (material.name.includes('UV_BAKED_GROUND'))
                    pbr.emissiveIntensity = 0.23;
...
function updateLabels() {
    campusTargets.forEach((target) => {
        ...
        if (value)
            value.textContent = target.power();
    });
}
function addAlarmBeacon() {
    ...
    const shell = new THREE.Mesh(new THREE.OctahedronGeometry(0.29, 0), new THREE.MeshBasicMaterial({
        color: 0xff7892,
        wireframe: true,
        transparent: true,
        opacity: 0.72,
...
function addEnergyDispatchHub(position) {
    ...
    const core = new THREE.Mesh(new THREE.SphereGeometry(0.135, 18, 14), new THREE.MeshBasicMaterial({
        color: 0x3eeaff,
        transparent: true,
        opacity: 0.66,
        blending: THREE.AdditiveBlending,
    ...
    const beacon = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.12, 3.8, 18, 1, true), new THREE.MeshBasicMaterial({
        color: 0x00dfff,
        transparent: true,
        opacity: 0.22,
        blending: THREE.AdditiveBlending,
...
function addCampusBoundaryGlow() {
    ...
    for (let index = 0; index < 112; index += 1) {
        ...
        const local = (t * 4) % 1;
        if (side === 0)
            nodes.push(-27 + local * 54, 0.42, -19);
        if (side === 1)
            nodes.push(27, 0.42, -19 + local * 38);
        if (side === 2)
            nodes.push(27 - local * 54, 0.42, 19);
        if (side === 3)
            nodes.push(-27, 0.42, 19 - local * 38);
...
function addOrthogonalFlow(kind, color, points) {
    const group = new THREE.Group();
    ...
    const isDispatchHubPoint = (point) => (point.y > 8
    ...
    [0, points.length - 1].forEach((index) => {
        if (isDispatchHubPoint(points[index]))
            return;
    ...
    routePoints.forEach((point, index) => {
        ...
        const ring = new THREE.Mesh(new THREE.TorusGeometry(terminal ? 0.32 : 0.22, terminal ? 0.035 : 0.025, 6, 32), nodeMaterial);
        ring.rotation.x = Math.PI / 2;
    ...
    const routePower = () => {
        if (kind === 'pv')
            return THREE.MathUtils.clamp(props.pvPowerKw / 280, 0.38, 1);
        if (kind === 'charging')
            return THREE.MathUtils.clamp(props.chargerPowerKw / 170, 0.38, 1);
        if (kind === 'grid')
            return THREE.MathUtils.clamp((props.loadPowerKw - props.pvPowerKw * 0.55) / 420, 0.38, 1);
        if (kind === 'load')
            return THREE.MathUtils.clamp(props.loadPowerKw / 560, 0.38, 1);
...
function modeForTarget(target) {
    if (target.code === 'STORAGE')
        return 'storage';
    if (target.code === 'CHARGING')
        return 'charging';
    if (/^[A-E]\d$/.test(target.code))
        return 'building';
    return null;
}
function focusTarget(target, mode = modeForTarget(target)) {
...
function applySceneMode() {
    ...
    trackedMeshes.forEach((mesh) => {
        ...
        if (activeKind === 'pv')
            active = name.includes('_PV_') || name.startsWith('BUILDING_');
        if (activeKind === 'storage')
            active = name.includes('STORAGE');
        if (activeKind === 'charging')
            active = name.includes('CHARG');
        if (activeKind === 'building')
            active = name.startsWith('BUILDING_') || name.includes('FACADE') || name.includes('WINDOW');
        if (activeKind === 'alarm')
            active = name.includes('C3');
        materials.forEach((material) => {
            const pbr = material;
    ...
    scene.children.forEach((child) => {
        const kind = child.userData.flowKind;
        if (!kind)
            return;
        child.visible = true;
    });
...
function applyTheme() {
    ...
    if (bloomPass)
        bloomPass.strength = isNight ? 0.72 : 0.4;
}
...
async function beginSceneWarmup() {
    if (!props.warmupRequested || loading.value || !renderer || !composer || sceneWarmupStarted)
        return;
...
function resizeScene() {
    if (!renderer || !composer || !labelRenderer || !sceneRef.value)
        return;
...
function animate(now = performance.now()) {
    if (!staticQa || loading.value)
        frame = requestAnimationFrame(animate);
    const lightweightIntro = props.introActive && !props.warmupRequested;
    const minimumFrameDuration = lightweightIntro ? 1000 / 24 : 0;
    if (minimumFrameDuration && now - lastRender < minimumFrameDuration)
        return;
    lastRender = now;
...
if (__VLS_ctx.selectedTarget) {
    ...
    __VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({});
    __VLS_asFunctionalElement(__VLS_intrinsicElements.span, __VLS_intrinsicElements.span)({});
    (__VLS_ctx.selectedTarget.code);
    __VLS_asFunctionalElement(__VLS_intrinsicElements.strong, __VLS_intrinsicElements.strong)({});
    (__VLS_ctx.selectedTarget.name);
    ...
    (__VLS_ctx.selectedTarget.type);
    __VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({});
    __VLS_asFunctionalElement(__VLS_intrinsicElements.dt, __VLS_intrinsicElements.dt)({});
    __VLS_asFunctionalElement(__VLS_intrinsicElements.dd, __VLS_intrinsicElements.dd)({});
    (__VLS_ctx.selectedTarget.power());
    __VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({});
    __VLS_asFunctionalElement(__VLS_intrinsicElements.dt, __VLS_intrinsicElements.dt)({});
    __VLS_asFunctionalElement(__VLS_intrinsicElements.dd, __VLS_intrinsicElements.dd)({
        ...{ class: "healthy" },
...
/** @type {__VLS_StyleScopedClasses['scene-loading']} */ ;
/** @type {__VLS_StyleScopedClasses['device-detail-card']} */ ;
